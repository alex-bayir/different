...
#et=$(date +"%FT%H:%M:%SZ")
stf=$(date +"%FT%H:%M:%SZ" -d $st | sed -e 's/T/ /g' -E -e 's/:[0-9][0-9]Z//g') # +UTC TimeZone and format
etf=$(date +"%FT%H:%M:%SZ" -d $et | sed -e 's/T/ /g' -E -e 's/:[0-9][0-9]Z//g') # +UTC TimeZone and format
...
alertstable=$(echo $alerts | jq -r '"|pr|                                  rule                                  | count |\n|--|------------------------------------------------------------------------|-------|\n" + (map("|\(.priority)|\(.rule[0:72] | (. + " " * (72 - length)))|\(.count | tostring | (" " * (7 - length) + .))|") | join("\n")) +"\n|"+ (("Итого: ☢️x" + ([.[] | select(.priority == "☢️") | .count] | add | tostring | sub("null";"0")) + " 🔴x" + ([.[] | select(.priority == "🔴") | .count] | add | tostring | sub("null";"0")) + " 🟡x" + ([.[] | select(.priority == "🟡") | .count] | add | tostring | sub("null";"0")) + " 🟢x" + ([.[] | select(.priority == "🟢") | .count] | add | tostring | sub("null";"0"))) | (. + " " * (71 - length))) + "|" + ([.[].count] | add | tostring | (" " * (7 - length) + .)) + "|"')
alertscsv=$(echo $alerts | jq -r '.[] | [.count,.priority,.rule] | @csv' | sed -ze 's/[\\\/]/\\\\\\&/g' -e 's/\n/\\n/g')
...
cat "$dir/report-template.html" | sed -e "s/{{events-date-export}}/$(echo $stf - $etf)/g" | sed -e "s/{{events-data-export}}/$eventscsv/g" -e "s/{{alerts-data-export}}/$alertscsv/g" > "$dir/report.html"

curl $([ $PROXY ] && echo "--proxy $PROXY") -X POST "https://api.telegram.org/bot$TG/sendDocument?chat_id=$CHAT&message_thread_id=$THREAD" -F document=@/opt/kaspersky/kuma/core/scripts/report.html
